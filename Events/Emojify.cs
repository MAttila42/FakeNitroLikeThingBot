    ...
    class Emojify
    {
        public async static void DoEvent()
        {
            ...
            foreach (var i in m)
            {
                ...
                emote = Program._client.Guilds.Where(x => (x.Name.Contains("Emojis") && x.OwnerId == BaseConfig.GetConfig().OwnerID) || x.Id == ((SocketGuildChannel)message.Channel).Guild.Id).SelectMany(x => x.Emotes).FirstOrDefault(x => x.Name == i);
